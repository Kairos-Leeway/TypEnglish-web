...
interface WordSlot {
  index: number; word: string; wordId: number | null; visible: boolean
  punctuation: boolean
...
const slotResults = ref<Array<{word:string;wordId:number|null;correct:boolean;answer:string}>>([])
...
function buildSlots(wordsData: Array<{index:number;word:string;wordId:number|null;translation?:string;phonetic?:string;punctuation?:boolean}>): WordSlot[] {
  const result: WordSlot[] = wordsData.map((w) => {
    ...
    const visible = isPunct || w.word.length <= 2 || Math.random() > 0.4
  ...
  if (result.every(s => s.visible) && result.length > 2) {
...
async function loadSentences() {
  ...
  try {
    ...
    if (data.length > 0) initSlots(data[0].words)
...
function validateSlot(slot: WordSlot): boolean {
  if (slot.status === 'correct' || slot.visible) return true
...
function submitBatchResults() {
  ...
  api.post('/practice/submit-batch', { mode: 'cloze', language: language.value, items }).catch(() => {})
}

function checkAllDone() {
  if (slots.value.filter(s => !s.visible).every(s => s.status === 'correct')) {
    submitBatchResults()
    celebrate()
    gainXp(30)
...
function recordSlot(slot: WordSlot, correct: boolean) {
  ...
  slotResults.value.push({ word: wordText, wordId: slot.wordId, correct, answer: slot.userInput })
}
...
function recordRemainingSlots() {
  for (const s of slots.value) {
    if (!s.visible && s.status !== 'correct') {
      ...
      slotResults.value.push({ word: wordText, wordId: s.wordId, correct: false, answer: s.userInput || '(未填)' })
    }
  }
}

function onKeydown(e: KeyboardEvent, slot: WordSlot, idx: number) {
  playKeytap()
  // Space / Tab / ArrowRight: 校验当前 → 跳到下一个空
  if (e.key === ' ' || e.key === 'Tab' || e.key === 'ArrowRight') {
    e.preventDefault()
    if (!slot.visible) validateSlot(slot)
    moveToSlot(idx, 1)
    return
  }
  // ArrowLeft: 回到上一个空
  ...
  if (e.key === 'Enter') {
    e.preventDefault()
    if (!slot.visible) { const ok = validateSlot(slot); if (ok) moveToSlot(idx, 1) }
  }
}

function showHint() {
  const s = slots.value.find(s => !s.visible && (s.status === 'pending' || s.status === 'retry'))
  if (!s) return
...
function hintLabel(slot: WordSlot): string {
  if (slot.hintLevel === 0) return ''
  const w = slot.word
  let shown = w.substring(0, slot.hintLevel)
...
function submitSentenceError() {
  ...
  if (!sent) return
  const hiddenSlots = slots.value.filter(s => !s.visible)
  const totalSlots = hiddenSlots.length
  const correctSlots = hiddenSlots.filter(s => s.status === 'correct').length
  if (totalSlots === correctSlots) return
  const slotResults = slots.value.map(s => ({
    index: s.index, word: s.word, visible: s.visible || s.punctuation,
  ...
  api.post('/practice/sentence-error', {
    ...
    mode: 'cloze', totalSlots, correctSlots, slotResults
  }).catch(() => {})
}
...
<template>
  <div class="cloze">
    <header class="topbar">
      ...
      <span class="page-title">TypEnglish · 完形填空</span>
      <div style="flex:1"/>
    ...
    <div v-else-if="finished" class="result-panel">
      ...
      <h2>练习完成</h2>
      <p class="result-meta">正确 {{correctCount}} 个空</p>
      <div class="result-actions">
        <router-link to="/" class="retry-btn" style="display:inline-block;text-decoration:none">返回首页</router-link>
        <button class="retry-btn secondary" @click="loadSentences()">再来一轮</button>
      </div>
    </div>
        ...
        <div class="mid-zone">
          ...
          <div class="slots-line">
            <template v-for="(s,i) in slots" :key="i">
              ...
              <span v-else-if="!s.visible" class="slot-wrapper" @mouseenter="s.status==='correct' ? onWordHover(s) : null" @mouseleave="s.status==='correct' ? onWordLeave() : null">
                <div v-if="hintLabel(s)" class="hint-above">{{ hintLabel(s) }}</div>
                <input :data-slot="s.index" v-model="s.userInput"
                  :class="['slot-input', s.status, {shaking:s.shaking}]" :disabled="s.status==='correct'"
                  :style="{width:Math.max(s.word.length*14+20,56)+'px'}"
                  spellcheck="false" autocomplete="off" @keydown="onKeydown($event,s,i)"
                />
              ...
              <span v-else class="vis-word" @mouseenter="onWordHover(s)" @mouseleave="onWordLeave">
                {{ s.word }}
...
<style scoped>
.cloze{height:100vh;height:100dvh;display:flex;flex-direction:column;overflow:hidden;background:#fef9f4}
...
.card{flex:1;display:flex;flex-direction:column;overflow-y:auto;padding:80px 24px 28px}
